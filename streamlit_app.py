...
# ---local imports---
from scraper import scrape_urls,scrape_urls_manually
...
with st.sidebar.container():
    ...
    with col1:
        ...
        url_text = st.text_area("Enter one or more URLs (space/tab/newline separated and use have http:,https:):",st.session_state["text_temp"], key="url_text_input", height=68)

    with col2:
        if st.button("Add URLs"):
            if url_text.strip():
                new_urls = re.split(r"\s+", url_text.strip())
                valid_urls = [u for u in new_urls if re.match(r'^(http://|https://)', u)]
                # Extend the session state list with only valid URLs
                st.session_state["urls_splitted"].extend(valid_urls)
                st.session_state["text_temp"] = ""
                st.rerun()
...
# Depth and Pagination Toggle Checkbox
show_depth_pagination = st.sidebar.toggle("Enable Depth and Pagination (Optional)", help="Check to enable settings for depth and pagination. This allows you to set how many pages to scrape and the selector for 'Next' or 'Load More' button.")

# Show Depth and Pagination settings if checkbox is checked
if show_depth_pagination:
    # Depth Input Section
    st.sidebar.subheader("Pagination Depth")
    depth_input = st.sidebar.number_input(
        "Enter Depth (Pagination Limit):", 
        min_value=0, 
        max_value=5, 
        value=0, 
        step=1, 
        help="Specify the number of pages or iterations you want to scrape.0 for Page the provided. 1 for Link within the Page ...",
        key="depth_input"
    )
    
    st.sidebar.subheader("Max Url")
    depth_input = st.sidebar.number_input(
        "Enter Max url (All Links might take time):", 
        min_value=1, 
        value=1, 
        step=1, 
        help="Specify the number of Max Url or iterations you want to scrape. This defines how maximum url the scraper do .",
        key="max_url"
    )

    st.sidebar.markdown("---")

    # Next Button Input (ID or Class)
    st.sidebar.subheader("Next Button (Pagination/Load More)")
    next_button_selector = st.sidebar.text_input(
        "Enter ID or Class of 'Next' Button:", 
        help="Provide the unique ID or class selector for the 'Next' or 'Load More' button. The scraper will automatically click this button to load more data.",
...
predefined_fields =  ['Email', 'Mobile Number', 'DOB', 'Name', 'Password']

fields = []

css_selectors = {}

# Display the checkboxes for predefined fields


# # Display text input for manual field (regex) if the user wants to add one
# custom_field = st.sidebar.text_input("Add Custom Field (Regex pattern, optional):", "")

# # Append the custom field to the list if it's not empty
# if custom_field:
#     fields.append(custom_field)

# # Show the fields that have been selected or added
# st.sidebar.write("Selected Fields:")
# for field in fields:
#     st.sidebar.write(f"- {field}")
fields = []
if show_tags:
    
    show_column_2 = st.sidebar.toggle("Use Ai Scrapper")
    # col1, col2 = st.sidebar.columns(2)
    if not show_column_2:
        st.sidebar.subheader("Select Fields to Extract:")
        for field in predefined_fields:
                    if st.sidebar.checkbox(f"Extract {field}", key=f"{field}_checkbox (uses RegEx )"):
                        fields.append(field.lower())  # Add selected field to the fields list
                        
                        # For each selected field, show an input for the CSS selector
                        css_selector = st.sidebar.text_input(f"CSS selector for {field} (Optional)", key=f"{field}_selector")
                        if css_selector:
                            # Save the CSS selector for the field in the dictionary
                            css_selectors[field.lower()] = css_selector
        if st.sidebar.button("Start Scraping", type="primary"):
            if st.session_state["urls_splitted"] == []:
                ...
                depth_value = st.session_state.get("depth_input", 0)  # Default to 1 if not set
                next_button_selector = st.session_state.get("next_button_selector", "")
                max_url= st.session_state.get('max_url',1)

                # Save user choices
                st.session_state['urls'] = st.session_state["urls_splitted"]
                st.session_state['fields'] = fields
                st.session_state['model_selection'] = model_selection
                st.session_state['scrap-type']="manual"
                st.session_state['css-selectors']= css_selectors
                st.session_state['use_pagination'] = False
                st.session_state['pagination_details'] = False
                # st.session_state['pagination_depth'] = depth_value  # Save the depth value
                # st.session_state['next_button_selector'] = next_button_selector  # Save the next button selector
                
                # fetch or reuse the markdown for each URL
                unique_names = fetch_and_store_markdowns(st.session_state["urls_splitted"],depth_value,max_url,next_button_selector)
    ...
    if show_column_2:
        ...
        if st.sidebar.button("Scrape", type="primary"):
            if st.session_state["urls_splitted"] == []:
                # print(st.session_state)
                st.error("Please enter at least one URL.")
            elif show_tags and len(fields) == 0:
                ...
                st.session_state['fields'] = fields
                st.session_state['model_selection'] = model_selection
                st.session_state['scrap-type']="ai"
                st.session_state['css-selectors']= css_selectors
                st.session_state['use_pagination'] =  False
                st.session_state['pagination_details'] = False
                # st.session_state['pagination_depth'] = depth_value  # Save the depth value
    ...
    # Help Button for Next Button

st.sidebar.markdown("---")

scheduling= st.sidebar.toggle("Enable Scheduling")
if scheduling:
    st.sidebar.subheader("Schedule Scraping Task")
    st.sidebar.markdown(
        "You can schedule when the scraping task should run. Choose from a predefined schedule or set a custom cron schedule."
    )
    ...
    cron_options = [
        ...
        ("Every month at midnight on the 1st", "0 0 1 * *"),
        ("Custom Schedule", "custom")
    ]

    # Dropdown to select a schedule frequency
    selected_schedule = st.sidebar.radio(
        "Select a schedule for scraping:", 
        [option[0] for option in cron_options],
        help="Choose when the scraping task should run. You can select a predefined schedule or create a custom one."
    )

    # Custom Schedule Inputs (if selected)
    if selected_schedule == "Custom Schedule":
        st.sidebar.markdown("### Custom Cron Schedule")
        st.sidebar.text("Set your own cron schedule by selecting values for each field.")
        
        minute = st.sidebar.selectbox("Minute", [str(i) for i in range(0, 60)], index=0, help="Choose the minute when the task should run.")
        hour = st.sidebar.selectbox("Hour", [str(i) for i in range(0, 24)], index=0, help="Choose the hour when the task should run.")
        day_of_month = st.sidebar.selectbox("Day of the Month", [str(i) for i in range(1, 32)] + ["*"], index=0, help="Choose the day of the month when the task should run.")
        month = st.sidebar.selectbox("Month", [str(i) for i in range(1, 13)] + ["*"], index=0, help="Choose the month when the task should run.")
        day_of_week = st.sidebar.selectbox("Day of the Week", [str(i) for i in range(0, 7)] + ["*"], index=0, help="Choose the day of the week when the task should run.")

        # Compile the custom cron syntax
        cron_syntax = f"{minute} {hour} {day_of_month} {month} {day_of_week}"

        # Button to Compile Cron
        

    else:
        # Show predefined cron syntax based on the user's selection
        cron_syntax = dict(cron_options)[selected_schedule]
        st.sidebar.markdown(f"**Scheduled Cron Syntax:** `{cron_syntax}`")
    
    if st.sidebar.button("Compile Cron"):
        if st.session_state["urls_splitted"] == []:
            print(st.session_state)
            st.error("Please enter at least one URL.")
        elif not show_tags:
            ...
            st.session_state['use_pagination'] =  False
            st.session_state['pagination_details'] = False
            st.session_state['cron-syntax']= cron_syntax
            # st.session_state['pagination_depth'] = depth_value  # Save the depth value
            # st.session_state['next_button_selector'] = next_button_selector  # Save the next button selector
            
            # fetch or reuse the markdown for each URL
            createCron(st.session_state['urls'],cron_syntax,fields,st.session_state['scrap-type'],css_selectors)
            # run_crons
                                    

            # job = cron.new(command=f'python3 -c "{fetch_and_store_markdowns(st.session_state["urls_splitted"],depth_value,max_url,next_button_selector)}"')

            # Set the custom cron expression
            # job.setall(cron_syntax)
    ...
    # Display the generated cron expression
    st.sidebar.markdown("### Current Cron Expression")
    st.sidebar.markdown(f"**Generated Cron Expression:** `{cron_syntax}`")
...
if st.session_state['scraping_state'] == 'scraping':
    try:
        with st.spinner("Processing..."):
            ...
            # 1) Scraping logic
            all_data = []
            print("scrapethpe",st.session_state['scrap-type'])
            print("showtags",show_tags)
            if show_tags and st.session_state['scrap-type']=='ai':
            ...
            if st.session_state['use_pagination']:
                in_tokens_p, out_tokens_p, cost_p, page_results = paginate_urls(unique_names, st.session_state['model_selection'],st.session_state['pagination_details'] or False,st.session_state["urls_splitted"])
